...
def save_test_task_id(pipeline: PipelineController, node: PipelineController.Node):
    pipeline.add_parameter("test_tasks_ids", default=pipeline.get_parameters()["test_tasks_ids"] + [node.executed])
    return
...
def main():
    ...
    pipe = PipelineController(name="pipeline",
        ...
        target_project="e-muse/DL_Wheat_with_clearml/pipeline_tasks",
        version="0.0.1",
    ...
    global_seed, rep_num, children_seeds = task.connect({"seed": global_seed, "number_of_repetitions": rep_num, "children_seeds": children_seeds}, "config").values()

    task.execute_remotely("services")
    
    pipe.add_parameter(name="test_tasks_ids", default=[])
    aggregator_parents = []
    ...
    for i, seed in enumerate(children_seeds):
        ...
        pipe.add_step(name=hpo_task_name, 
                    ...
                    parameter_override={"tasks_seed/seed": seed},
    ...
    pipe.add_step(name="pipeline aggregator", 
                ...
                parameter_override={"tasks_ids/test_tasks_to_aggregate": "${pipeline.test_tasks_ids}"},
