...
def multi_layer_perceptron(X_train, y_train, X_test, y_test):
    mlp = MLPRegressor(max_iter=150, early_stopping = True)   #---------
    ...
    clf = GridSearchCV(mlp, parameter_space, n_jobs=10, cv=5)
...
def main():
    task = clearml.Task.init(project_name="e-muse/DL_Wheat_with_clearml/tests", 
                                           task_name="original code",
                                           task_type="training",
   ...
   'megengine': True, 'catboost': True
    ...
    # data = pd.read_csv('DL/NAM_dat(dataset).csv', header=0)
    data = pd.read_csv('/data/NAM_dat(dataset).csv', header=0)
    dataset=data.iloc[:,60:]
    # target=data.iloc[:, 4]
    # target=data.loc[:, "2016_Yield"]
    target=data.loc[:, "2014_Height"]
